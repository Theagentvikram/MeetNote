...
const SWIFT_AUDIO_CAPTURE_SOURCE = path.join(__dirname, '../MeetNote/Services/AudioCaptureService.swift');
const SWIFT_APP_SETTINGS_SOURCE  = path.join(__dirname, '../MeetNote/Models/AppSettings.swift');
...
const SWIFT_BRIDGE_ENTITLEMENTS = IS_PACKAGED
  ...
  : path.join(__dirname, 'backend/desktop-app/entitlements.mac.plist');
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('show-recording-overlay', async () => {
    ...
    recordingOverlayWindow.loadFile(path.join(__dirname, 'backend/desktop-app/src/overlay.html'));
  ...
  ipcMain.handle('show-coach-overlay', async () => {
    ...
    coachOverlayWindow.loadFile(path.join(__dirname, 'backend/desktop-app/src/coach-overlay.html'));
